...
if [ $PPPOECONN = true ] ; then
    ...
    echo -e "\033[92mPPPoE username:\033[93m $PPPUSER\033[0m" | sudo tee /dev/tty1
    echo -e "\033[92mPPPoE password:\033[93m $PPPPASS\033[0m" | sudo tee /dev/tty1
else
    echo -e "\033[92mInternet Access:\033[93m Disabled\033[0m" | sudo tee /dev/tty1
fi
echo -e "\033[92mPi hostname:\033[93m $HOSTNAME\033[0m" | sudo tee /dev/tty1
echo -e "\033[92mPPPwn timeout (whole time before a new attempt):\033[93m $TIMEOUT\033[0m" | sudo tee /dev/tty1
if [ $PPDBG = true ] ; then
    echo -e "\033[92mPPPwn verbose (debug):\033[93m Enabled\033[0m\n" | sudo tee /dev/tty1
else
    echo -e "\033[92mPPPwn verbose (debug):\033[93m Disabled\033[0m\n" | sudo tee /dev/tty1
fi
...
echo -e "\033[95mReady for console connection\033[0m\n" | sudo tee /dev/tty1
while [ true ] ; do
    if [ -f /boot/firmware/PPPwn/config.sh ]; then
