...
while true; do
	read -p "$(printf '\r\n\r\n\033[36mDo you want to change the firmware version being used, the default is 11.00\r\n\r\n\033[36m(Y|N)?: \033[0m')" fwset
	case $fwset in
		[Yy]* )
			while true; do
...
ip link
while true; do
	read -p "$(printf '\r\n\r\n\033[36mDo you want to change the pi lan interface, the default is eth0\r\n\r\n\033[36m(Y|N)?: \033[0m')" ifset
	case $ifset in
		[Yy]* )
			while true; do
...
while true; do
	read -p "$(printf '\r\n\r\n\033[36mDo you want to change the hostname, the default is pppwn\r\n\r\n\033[36m(Y|N)?: \033[0m')" hstset
	case $hstset in
		[Yy]* )
			while true; do
...
while true; do
	read -p "$(printf '\r\n\r\n\033[36mDo you want to enable PPPwn verbose (PPPwn debug)\r\n\r\n\033[36m(Y|N)?: \033[0m')" ppdbg
	case $ppdbg in
		[Yy]* )
			PPDBG="true"
...
while true; do
	read -p "$(printf '\r\n\r\n\033[36mDo you want to change the timeout for the PPPwn process to give up and try again (if stuck), the default is 5 minutes\r\n\r\n\033[36m(Y|N)?: \033[0m')" pptout
	case $pptout in
		[Yy]* )
			while true; do
				read -p  "$(printf '\033[33mEnter the timeout: \033[0m')" TIMEOUT
				case $TIMEOUT in
					"" )
						echo -e '\033[31mCannot be empty!\033[0m';;
					"0" )
						echo -e '\033[31mCannot be zero!\033[0m';;
					* )
						if grep -q '^[a-zA-Z_ -]*$' <<<$TIMEOUT ; then
							echo -e '\033[31mThe timeout must only contain numbers\033[0m';
						else
							break;
						fi
				esac
			done
			echo -e '\033[32mYou set the timeout for '$TIMEOUT'm ('$TIMEOUT' minutes)\033[0m'
			TIMEOUT=${TIMEOUT}m
			break;;
		[Nn]* )
			echo -e '\033[35mUsing the default setting: 5m (5 minutes)\033[0m'
			TIMEOUT='5m'
			break;;
		* ) echo -e '\033[31mPlease answer Y or N\033[0m';;
	esac
