...
ACTION=="remove", SUBSYSTEM=="block", RUN+="/boot/firmware/PPPwn/pwnumount.sh $kernel"' | sudo tee /etc/udev/rules.d/99-pwnmnt.rules
	...
	while true; do
		...
		*) echo -e '\033[31mPlease answer Y or N\033[0m' ;;
		...
		while true; do
		read -p "$(printf '\r\n\r\n\033[36mDo you want to change the default redirect link on the webserver? (The default is http://karo218.ir/1100)\r\n\r\n\033[36m(Y|N)?: \033[0m')" rdrct
		case $rdrct in
		[Yy]*)
						while true; do
				read -p "$(printf '\033[33mEnter the link (http://example.com): \033[0m')" REDIRECT
				case $REDIRECT in
				"")
					echo -e '\033[31mCannot be empty!\033[0m'
					;;
				*)
					if [[ ! $REDIRECT =~ ^http:// ]]  then
							echo -e '\033[31mThe URL must be formatted like http://example.com\033[0m'
					else
						echo -e '\033[33mThe URL is valid\033[0m'
						break
					fi
					;;
				esac
			done
			echo -e '\033[32mYou are using the redirect link: '$REDIRECT', for the redirect button\033[0m'
			break
			;;
		[Nn]*)
			echo -e '\033[35mThe default link (http://karo218.ir/1100) will be used\033[0m'
...
TIMEOUT="'$TOUT'm"
PYPWN='$UPYPWN'
REDIRECT="'$REDIRECT'"' | sudo tee /boot/firmware/PPPwn/config.sh
